...
export default function UkHeader({
  ...
  useEffect(() => {
    ...
    loadCartCount();
    window.addEventListener("cartUpdated-uk", loadCartCount);
    return () => window.removeEventListener("cartUpdated-uk", loadCartCount);
  }, []);
  ...
  return (
    <header className="siteHeaderLucknawi">
      {/* 1. Top Announcement Bar for UK */}
      <div className="announcementBarWrapper">
        <div className="announcementBarInner">
          <p className="announcementText">
            <span>Free Tracked UK Delivery on orders over £75</span>
            <span className="announcementSeparator">·</span>
            <span>Standard UK Delivery £4.99 (2–4 Business Days)</span>
          </p>
        </div>
      </div>
      ...
      <div className="headerMiddleRow">
        <div className="headerLeftActions">
          <button
            ...
            aria-controls="uk-site-navigation"
          >
            {mobileOpen ? <X size={24} /> : <Menu size={24} />}
          </button>
        </div>

        <a href="/uk" className="headerBrandLogo" aria-label="Bustaniya UK Home">
      ...
      {/* 3. Bottom Centered Navigation Bar (UK Scoped Links) */}
      <nav id="uk-site-navigation" className={`headerNavRow ${mobileOpen ? "mobileOpen" : ""}`}>
        ...
        {navigationCategories.map((category) => (
          <a
            ...
            href={`/uk/category/${category.slug}`}
            key={category.slug}
          >
