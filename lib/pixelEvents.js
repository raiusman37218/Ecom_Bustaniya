...
export async function getPixelFunnelSummary({ days = 7 } = {}) {
  ...
  try {
    const [eventsRes, ordersRes] = await Promise.all([
      supabaseAdminRequest(
        `pixel_events?select=id,event_name,event_id,source,success,em_hash,ph_hash,value,error_message,http_status,client_ip,user_agent,event_source_url,created_at&created_at=gte.${encodeURIComponent(since)}&limit=50000`
      ),
      supabaseAdminRequest(
        `orders?select=id,status,courier_status,payment_status,total_pkr,created_at&created_at=gte.${encodeURIComponent(since)}&limit=5000`
      ).catch(() => []),
    ]);
  ...
  rows.forEach((row) => {
    ...
    const sessionKey = `${row.client_ip || "ip"}_${(row.user_agent || "").slice(0, 30)}_${row.em_hash || ""}`;
    sessionFingerprints.add(sessionKey);
  ...
  // Calculate refunds & returns from operational order data
  let refundsValue = 0;
  let refundsCount = 0;
  let returnsCount = 0;
  ...
  orderRows.forEach((order) => {
    ...
    if (isRefund) {
      refundsCount += 1;
      refundsValue += Number(order.total_pkr || 0);
    }
  ...
  const netRevenue = Math.max(0, grossPurchaseValue - refundsValue);
  ...
  // Ecommerce operator rates
  const uniqueSessions = Math.max(sessionFingerprints.size, counts.PageView || 0, 1);
  const overallConversionRate = Number(((counts.Purchase / uniqueSessions) * 100).toFixed(2));
  const addToCartRate = Number(((counts.AddToCart / uniqueSessions) * 100).toFixed(2));
  const checkoutCompletionRate = counts.InitiateCheckout > 0
    ? Number(((counts.Purchase / counts.InitiateCheckout) * 100).toFixed(1))
    : counts.Purchase > 0 ? 100 : 0;
  ...
  // - Critical: missing credentials or EMQ 0
  // - Warning: delivery rate below 95%
  // - Healthy: delivery >= 95% & connected
  ...
  return {
    ...
    revenue: {
      grossPurchases: grossPurchaseValue,
      netRevenue,
      refundsValue,
      refundsCount,
      returnsCount,
    ...
    purchaseValue: grossPurchaseValue,
