...
const POSTEX_PAYMENT_URL =
  "https://api.postex.pk/services/integration/api/order/v1/payment-status";
...
function expectedNet(payment) {
  return amount(
    Number(payment?.invoice_payment_pkr || 0) -
      Number(payment?.transaction_tax_pkr || 0) -
      Number(payment?.transaction_fee_pkr || 0) -
      Number(payment?.reversal_tax_pkr || 0) -
      Number(payment?.reversal_fee_pkr || 0)
  );
}
...
export function buildPostexSettlementSummary(payments = [], batches = [], items = []) {
  ...
  const totalExpectedNetPkr = amount(
    deliveredPayments.reduce((sum, payment) => sum + Number(payment.expected_net_pkr ?? expectedNet(payment)), 0)
  );
  const postexSettledPkr = amount(
    deliveredPayments
      .filter((payment) => payment.postex_settled)
      .reduce((sum, payment) => sum + Number(payment.expected_net_pkr ?? expectedNet(payment)), 0)
  );
  ...
  const outstandingPkr = amount(
    deliveredPayments.reduce((sum, payment) => {
      const expected = Number(payment.expected_net_pkr ?? expectedNet(payment));
...
async function syncOneOrder(order) {
  ...
  if (!order?.id || !isTrackingNumber(trackingNumber)) return null;
  const [trackingResult, paymentResult] = await Promise.all([
    postexJson(`${POSTEX_TRACK_URL}/${encodeURIComponent(trackingNumber)}`),
    postexJson(`${POSTEX_PAYMENT_URL}/${encodeURIComponent(trackingNumber)}`),
  ]);
...
export async function syncPostexPayments() {
  ...
  const eligible = orders.filter((order) => isTrackingNumber(text(order.courier_tracking_number)));
  const results = await Promise.allSettled(eligible.map(syncOneOrder));
  const rows = results
...
export async function savePostexCprBatch(input = {}) {
  ...
  const outstandingWeights = selected.map((payment) =>
    amount(
      Math.max(
        0,
        Number(payment.expected_net_pkr ?? expectedNet(payment)) -
