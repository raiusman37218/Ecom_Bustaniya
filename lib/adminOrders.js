...
export async function getAdminOrdersPage(options = {}) {
  ...
  const [items, operationResult, eventResult] = await Promise.all([
    orderIds.length
      ? supabaseAdminRequest(`order_items?select=*&order_id=in.(${orderIds.join(",")})&order=id.asc`)
      : Promise.resolve([]),
    getOrderOperations(orderIds),
    getOrderOperationEvents(orderIds),
  ]);
