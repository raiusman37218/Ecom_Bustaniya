...
import { refreshPostexOrderStatuses } from "./postexTracking";
...
export async function getAdminOrdersPage(options = {}) {
  ...
  const orders = await refreshPostexOrderStatuses(savedOrders);
  ...
  const operationResult = await getOrderOperations(orderIds);
  const eventResult = await getOrderOperationEvents(orderIds);
  return {
    orders: orders.map((order) => ({
      ...
      operation_events: eventResult.events.get(String(order.id)) || [],
    })),
    pagination: {
      page,
      pageSize,
