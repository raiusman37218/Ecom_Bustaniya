...
export function postexTrackingStatus(result) {
  ...
  const latestHistoryMsg = Array.isArray(history) && history.length > 0
    ...
    : "";
  const distStatus = text(result?.dist?.transactionStatus || result?.dist?.orderStatus || result?.transactionStatus || result?.orderStatus, 120);

  if (distStatus.toLowerCase().includes("deliver") || latestHistoryMsg.toLowerCase().includes("deliver")) {
    return "Delivered";
  }
  if (latestHistoryMsg && (
    latestHistoryMsg.toLowerCase().includes("out for delivery") ||
    latestHistoryMsg.toLowerCase().includes("enroute for delivery") ||
    latestHistoryMsg.toLowerCase().includes("waiting for delivery")
  )) {
    return "Out For Delivery";
  }
  if (latestHistoryMsg && latestHistoryMsg.toLowerCase().includes("return")) {
    return "Returned";
  }
  if (latestHistoryMsg && latestHistoryMsg.toLowerCase().includes("attempt")) {
    return "Attempted";
  }
  return distStatus || latestHistoryMsg;
}
