...
export function postexTrackingStatus(result) {
  const dist = result?.dist || {};
  const history = dist.transactionStatusHistory;
  const latestHistoryMsg = Array.isArray(history) && history.length > 0
    ? text(history[history.length - 1]?.transactionStatusMessage, 120)
    : "";
  const distStatus = text(dist.transactionStatus || dist.orderStatus || result?.transactionStatus || result?.orderStatus, 120);

  const dLower = distStatus.toLowerCase();
  const hLower = latestHistoryMsg.toLowerCase();

  // 1. Delivered check: strictly "delivered" or "delivered to customer"
  if (dLower === "delivered" || hLower === "delivered to customer" || hLower.includes("delivered to customer")) {
    return "Delivered";
  }

  // 2. Returns check: Returned vs Out For Return
  if (dLower === "returned" || hLower.includes("returned to merchant") || hLower === "returned") {
    return "Returned";
  }
  if (
    dLower.includes("return") ||
    hLower.includes("return") ||
    hLower.includes("en route to merchant") ||
    hLower.includes("return process")
  ) {
    return "Out For Return";
  }

  // 3. Delivery Under Review
  if (dLower.includes("under review") || hLower.includes("under review")) {
    return "Delivery Under Review";
  }

  // 4. Attempted
  if (dLower.includes("attempt") || hLower.includes("attempt") || hLower.includes("re-attempt")) {
    return "Attempted";
  }

  // 5. Out For Delivery
  if (
    dLower.includes("out for delivery") ||
    hLower.includes("out for delivery") ||
    hLower.includes("enroute for delivery") ||
    hLower.includes("waiting for delivery") ||
    hLower.includes("scheduled for delivery")
  ) {
  ...
  // 6. In Transit
  if (
    dLower.includes("transit") ||
    hLower.includes("transit") ||
    hLower.includes("departed to") ||
    hLower.includes("arrived at") ||
    hLower.includes("received at") ||
    hLower.includes("in-transit")
  ) {
    return "In-Transit";
  }

  // 7. Booked / Ready for pickup at merchant warehouse
  if (
    dLower === "unbooked" ||
    dLower === "booked" ||
    hLower.includes("warehouse") ||
    hLower.includes("booked") ||
    Boolean(dist.trackingNumber || result?.trackingNumber)
  ) {
    return "Booked";
  }

  return distStatus || latestHistoryMsg || "Booked";
}
