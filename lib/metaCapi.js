...
export async function sendMetaCapiEvent({
  ...
  let pixelId = DEFAULT_PIXEL_ID;
  let accessToken = DEFAULT_ACCESS_TOKEN;

  try {
    const storeSettings = await getStoreSettings();
    if (storeSettings?.domainSettings?.metaPixelId) {
      pixelId = storeSettings.domainSettings.metaPixelId;
    } else if (process.env.NEXT_PUBLIC_META_PIXEL_ID || process.env.META_PIXEL_ID) {
      pixelId = process.env.NEXT_PUBLIC_META_PIXEL_ID || process.env.META_PIXEL_ID;
    }

    if (storeSettings?.domainSettings?.metaCapiAccessToken) {
      accessToken = storeSettings.domainSettings.metaCapiAccessToken;
    } else if (process.env.META_CAPI_ACCESS_TOKEN) {
      accessToken = process.env.META_CAPI_ACCESS_TOKEN;
    }
  } catch {
    pixelId = process.env.NEXT_PUBLIC_META_PIXEL_ID || process.env.META_PIXEL_ID || DEFAULT_PIXEL_ID;
    accessToken = process.env.META_CAPI_ACCESS_TOKEN || DEFAULT_ACCESS_TOKEN;
  }
  ...
  try {
    ...
    try {
      const { recordMetaEvent } = await import("./metaEventsLog");
      recordMetaEvent({
        eventName,
        ...
        status: res.ok ? "Delivered to Meta (200 OK)" : `Meta API Error (${res.status})`,
        metaResponse: result,
      });
    } catch {}
