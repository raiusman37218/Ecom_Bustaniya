...
export async function updateAdminOrder({ orderId, input = {}, actor }) {
  ...
  // Confirmation is an order-lifecycle decision, not proof of payment. The
  // current policy allows every submitted order to be confirmed immediately;
  // payment proof remains independently tracked and can still block dispatch
  // for advance-payment orders through the gate above.
