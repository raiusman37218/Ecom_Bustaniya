...
export async function parseOrderUpdateRequest(request) {
  ...
  const allowed = new Set(["orderId", "orderStage", "paymentStatus", "fulfillmentStatus", "tracking", "notes", "tags", "operation", "paymentReference", "confirmationStatus"]);
...
export async function updateAdminOrder({ orderId, input = {}, actor }) {
  ...
  if ((dispatchStatuses.has(nextStatus) || input.fulfillmentStatus === "Booked with PostEx" || input.fulfillmentStatus === "Shipped") && verificationStatus !== "Payment Verified") {
  ...
  const updates = { ...(input.orderStage !== undefined ? { status: input.orderStage, courier_status: input.orderStage } : {}), ...(requestedPaymentStatus !== undefined ? { payment_status: requestedPaymentStatus, payment_proof_status: requestedPaymentStatus } : {}), ...(confirmationStatus !== undefined ? { order_confirmation_status: confirmationStatus } : {}), ...(paymentReference !== undefined ? { payment_reference: paymentReference || null } : {}), ...(input.fulfillmentStatus !== undefined ? { fulfillment_status: input.fulfillmentStatus } : verificationFulfillment ? { fulfillment_status: verificationFulfillment } : {}), ...(tracking !== undefined ? { courier_tracking_number: tracking, tracking_number: tracking } : {}), ...(input.notes !== undefined ? { internal_notes: text(input.notes, 2000) } : {}), ...(input.tags !== undefined ? { tags: incomingTags } : {}) };
  ...
  try {
    ...
    return { order, operation: nextOperation, coreOrderUpdated: true, operationPersistence: "saved", auditPersistence: "saved", inventoryRestore: restoreResult };
