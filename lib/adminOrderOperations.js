...
import { normalizeCourierStatus } from "./shipments";
...
export function cleanUserNote(rawNote = "") {
...
export function formatPaymentStatus(value = "", { advance = 0, total = 0 } = {}) {
...
export async function updateAdminOrder({ orderId, input = {}, actor }) {
  ...
  const effectiveTracking = tracking !== undefined ? tracking : (current.courier_tracking_number || current.tracking_number);
  ...
  const requestedStage = input.orderStage !== undefined ? input.orderStage : (input.postexStatus !== undefined ? input.postexStatus : input.status);
  const nextStatus = requestedStage !== undefined ? requestedStage : previousStatus;
  const isUnbooking = nextStatus && String(nextStatus).toLowerCase().includes("unbook");
  ...
  // The order total is the single base for every advance/COD split so the
  // admin table, the drawer and the PostEx invoicePayment can never disagree.
  const currentOrderTotal = Math.max(0, Number(current.total_order_value_pkr ?? current.total_pkr ?? 0) || 0);
  const advancePaidInput = input.amountPayableInAdvance === undefined
    ? undefined
    : Math.min(currentOrderTotal || Infinity, Math.max(0, Number(input.amountPayableInAdvance || 0) || 0));
  const updates = {
    ...(requestedStage !== undefined ? {
      status: requestedStage,
      courier_status: requestedStage,
      courier_raw_status: requestedStage,
      courier_normalized_status: isUnbooking ? "unassigned" : normalizeCourierStatus(requestedStage, { hasTracking: Boolean(effectiveTracking) }),
      ...(isUnbooking ? { fulfillment_status: "Unfulfilled" } : {}),
    } : {}),
    ...
    ...(advancePaidInput !== undefined ? { amount_payable_in_advance_pkr: advancePaidInput, amount_payable_on_delivery_pkr: Math.max(0, currentOrderTotal - advancePaidInput) } : {}),
    ...(input.fulfillmentStatus !== undefined ? { fulfillment_status: input.fulfillmentStatus } : verificationFulfillment && !isUnbooking ? { fulfillment_status: verificationFulfillment } : {}),
    ...(tracking !== undefined ? { courier_tracking_number: tracking || null, tracking_number: tracking || null } : {}),
  ...
  // Declared outside the items block on purpose: the totals recalculation
  // below reads it. A `let` inside the block made every order save throw
  // "calculatedProductSubtotal is not defined".
  ...
  if (calculatedProductSubtotal > 0) {
    // A free-delivery (full advance) order stores 0 delivery charges. `||`
    // read that as missing and silently re-added Rs. 250, so the saved total
    // and COD stopped matching what PostEx had been told to collect.
    const deliveryFee = Math.max(0, Number(current.delivery_charges_pkr ?? current.delivery_pkr ?? 250) || 0);
    const newTotal = calculatedProductSubtotal + deliveryFee;
    const advance = Math.max(0, Number(updates.amount_payable_in_advance_pkr ?? current.amount_payable_in_advance_pkr ?? 0) || 0);
    const cappedAdvance = Math.min(advance, newTotal);
    const totalUpdates = {
      ...
      total_pkr: newTotal,
      amount_payable_in_advance_pkr: cappedAdvance,
      amount_payable_on_delivery_pkr: Math.max(0, newTotal - cappedAdvance),
    };
