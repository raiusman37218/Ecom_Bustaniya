...
export async function updateAdminOrder({ orderId, input = {}, actor }) {
  ...
  if (Array.isArray(input.items) && input.items.length) {
    const passedDbIds = new Set(input.items.map((it) => it.id && !String(it.id).startsWith("fallback") && !String(it.id).startsWith("manual") && Number(it.id)).filter(Boolean));
    ...
    for (const item of input.items) {
      ...
      const validProductId = (item.productId || item.product_id) && UUID.test(String(item.productId || item.product_id)) ? String(item.productId || item.product_id) : null;
      const isExistingDbId = item.id && !String(item.id).startsWith("fallback") && !String(item.id).startsWith("manual") && !isNaN(Number(item.id));
