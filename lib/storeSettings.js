...
const VALID_SECTION_TYPES = ["hero", "new_arrivals", "shop_by_category", "our_story", "newsletter"];
...
function normalizeStoreTheme(value) {
  // Keep the theme identifier strict. More approved themes can be added here
  // without letting an invalid database value affect the storefront.
  return value === "editorial" ? "editorial" : "editorial";
}
...
function normalizeStoreSettings(record = {}, includeFinance = false) {
  ...
  const settings = {
    ...
    activeTheme: normalizeStoreTheme(announcementData.activeTheme),
...
export async function updateStoreSettings(settings = {}) {
  ...
  const activeTheme = normalizeStoreTheme(settings.activeTheme);
