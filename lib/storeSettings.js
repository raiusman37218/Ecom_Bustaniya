...
function normalizePaymentSettings(value) {
  ...
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const provider = ["payfast", "easypaisa", "jazzcash", "manual"].includes(source.provider) ? source.provider : defaults.provider;
  const advanceType = ["fixed", "percent"].includes(source.advanceType) ? source.advanceType : defaults.advanceType;
  return {
    codEnabled: source.codEnabled !== false,
    onlineGatewayEnabled: source.onlineGatewayEnabled === true,
    ...
    highRiskAdvanceEnabled: source.highRiskAdvanceEnabled !== false,
    advanceType,
    advanceAmountPkr: Math.max(0, Number(source.advanceAmountPkr ?? defaults.advanceAmountPkr)),
    advancePercent: Math.min(100, Math.max(0, Number(source.advancePercent ?? defaults.advancePercent))),
    bankTitle: String(source.bankTitle || "").trim().slice(0, 120),
    bankIban: String(source.bankIban || "").trim().slice(0, 80),
    jazzcashNumber: String(source.jazzcashNumber || "").trim().slice(0, 40),
    easypaisaNumber: String(source.easypaisaNumber || "").trim().slice(0, 40),
    instructions: String(source.instructions || defaults.instructions).trim().slice(0, 500),
  };
}
...
function normalizeCheckoutSettings(value) {
  ...
  return {
    ...
    defaultPayment: source.defaultPayment === "bank_deposit" ? "bank_deposit" : defaults.defaultPayment,
...
const VALID_SECTION_TYPES = ["hero", "new_arrivals", "shop_by_category", "our_story", "newsletter", "instagram_feed"];
...
function normalizeHomepageSections(value) {
  ...
  const normalized = value
    ...
    .map((item) => {
      const defaults = DEFAULT_HOMEPAGE_SECTIONS.find((d) => d.type === item.type) || {};
      return {
        id: String(item.id || defaults.id || `section-${item.type}`),
        type: item.type,
        enabled: item.enabled !== false,
        label: String(item.label || defaults.label || item.type).trim(),
...
function normalizeStoreSettings(record = {}, includeFinance = false) {
  ...
  const legacyDesktopHero = {
    eyebrow: String(record.hero_eyebrow || DEFAULT_STORE_SETTINGS.heroEyebrow).trim(),
    heading: String(record.hero_heading || DEFAULT_STORE_SETTINGS.heroHeading).trim(),
    supportingText: String(record.hero_supporting_text || "").trim(),
    primaryButtonText: String(record.hero_primary_button_text || DEFAULT_STORE_SETTINGS.heroPrimaryButtonText).trim(),
    primaryButtonLink: String(record.hero_primary_button_link || DEFAULT_STORE_SETTINGS.heroPrimaryButtonLink).trim(),
...
export async function updateStoreSettings(settings = {}) {
  ...
  const heroDesktopContent = normalizeHeroContent(settings.heroDesktopContent, {
    eyebrow: settings.heroEyebrow || DEFAULT_STORE_SETTINGS.heroEyebrow,
    heading: settings.heroHeading || DEFAULT_STORE_SETTINGS.heroHeading,
    supportingText: settings.heroSupportingText || "",
    primaryButtonText: settings.heroPrimaryButtonText || DEFAULT_STORE_SETTINGS.heroPrimaryButtonText,
    primaryButtonLink: settings.heroPrimaryButtonLink || DEFAULT_STORE_SETTINGS.heroPrimaryButtonLink,
    secondaryButtonText: settings.heroSecondaryButtonText || "",
    secondaryButtonLink: settings.heroSecondaryButtonLink || "",
  ...
  const record = {
    ...
    hero_heading: heroDesktopContent.heading || DEFAULT_STORE_SETTINGS.heroHeading,
    ...
    announcements: { items: announcements, sectionColors, sectionTextColors, heroDesktopContent, heroMobileContent, paymentSettings, shippingZones, notificationSettings, domainSettings, checkoutSettings, productCardStyle, categorySectionStyle, homepageSections: normalizeHomepageSections(settings.homepageSections), instagramEnabled: settings.instagramEnabled !== false, instagramHandle, instagramPosts, financeTransactions, financeAllocation, financeFixedCosts, financeManualExpenses, financePackagingExpense, financeDeliveryExpense, marketingCampaigns, productionBatches, inventorySources, inventoryMaterials, supplierBills },
