...
// The previous Bustaniya Meta dataset was retired. Keep this mapping here so
// the storefront and Admin never accidentally initialise the retired pixel
// from an older store_settings record.
...
function normalizeDomainSettings(value) {
  ...
  return {
    ...
    metaPixelId: configuredPixelId === RETIRED_META_PIXEL_ID ? ACTIVE_META_PIXEL_ID : configuredPixelId,
...
export async function getStoreSettings(options = {}) {
  ...
  try {
    const rows = await supabaseAdminRequest("store_settings?select=*&id=eq.1&limit=1", { revalidate: 60 });
