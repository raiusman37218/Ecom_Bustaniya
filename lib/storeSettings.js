...
function normalizeStoreSettings(record = {}, includeFinance = false) {
  ...
  if (includeFinance) settings.financeTransactions = normalizeFinanceTransactions(announcementData.financeTransactions);
  return settings;
}
...
export async function updateStoreSettings(settings = {}) {
  ...
  const record = {
    ...
    announcements: { items: announcements, financeTransactions },
