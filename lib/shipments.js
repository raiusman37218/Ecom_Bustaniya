...
export function normalizeCourierStatus(value = "", { hasTracking = false, manual = false } = {}) {
  ...
  if (status.includes("unbook") || status.includes("unassigned") || status.includes("un-assigned")) {
    return hasTracking ? "booked" : "unassigned";
  }
  // Strictly delivered or delivered to customer (must not match out for delivery)
  if (status === "delivered" || status.includes("delivered to customer")) return "delivered";
  if (
    status.includes("out for delivery") ||
    status.includes("enroute for delivery") ||
    status.includes("waiting for delivery") ||
    status.includes("scheduled for delivery")
  ) return "out_for_delivery";
  if (status.includes("return") || status.includes("en route to merchant")) return "returned";
  ...
  if (status.includes("hold") || status.includes("under review")) return "on_hold";
  ...
  if (status.includes("pending")) return hasTracking ? "booked" : "pending_booking";
  return hasTracking ? "booked" : "unassigned";
}
...
export async function recordShipmentState({ orderId, courier, trackingNumber = "", rawStatus = "", serviceType = "COD", manual = false }) {
  ...
  const normalizedStatus = normalizeCourierStatus(rawStatus, { hasTracking, manual });
  ...
  const fulfillmentPatch = isDelivered
    ? { fulfillment_status: "Delivered", payment_status: "Paid", delivered_at: new Date().toISOString() }
    : isReturned
      ? { fulfillment_status: "Returned", returned_at: new Date().toISOString() }
      : isOutForDelivery
        ? { fulfillment_status: "Out for delivery" }
        : isInTransit
          ? { fulfillment_status: "In transit" }
          : isBooked
            ? { fulfillment_status: "Booked with PostEx" }
            : {};
