...
function tempUsersFile() {
  return path.join(os.tmpdir(), "lisette-store", "admin-users.json");
}

function homeUsersFile() {
  return path.join(os.homedir(), ".lisette-store", "Lisette", "admin-users.json");
}

function defaultWritableUsersFile() {
  if (optionalEnv("ADMIN_USERS_FILE")) return optionalEnv("ADMIN_USERS_FILE");
  if (process.env.APPDATA) {
    return path.join(process.env.APPDATA, "Lisette", "admin-users.json");
  }
...
const DEFAULT_OWNER_EMAIL = "owner@lisette.local";
const DEFAULT_ADMIN_PASSWORD = "Lisette@1122";
let activeUsersFile = "";
